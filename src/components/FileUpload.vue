...
<template>
  <div class="space-y-4">
    ...
    <Card 
      ...
      class="border-2 border-dashed border-muted-foreground/25 p-8 text-center cursor-pointer hover:border-primary transition-colors group"
    >
      <CardContent class="p-0">
        <input 
          ...
          class="hidden" 
        />
        <UploadCloud class="mx-auto h-12 w-12 text-muted-foreground group-hover:text-primary transition-colors" />
        <p class="mt-4 text-foreground">
          <span class="font-semibold text-primary">Click to upload</span> or drag and drop
        </p>
        <p class="text-xs text-muted-foreground mt-1">JPG, PNG, WEBP, or GIF (max 15MB)</p>
      </CardContent>
    </Card>

    <!-- Selected Files List -->
    <div v-if="files.length > 0" class="space-y-3">
      <h4 class="text-sm font-medium text-muted-foreground">Selected Images:</h4>
      <ul class="space-y-2">
        <li 
          ...
          <Card class="flex-1">
            <CardContent class="p-3">
              <div class="flex items-center gap-3 truncate">
                <FileImage class="h-5 w-5 flex-shrink-0 text-muted-foreground" />
                <span class="text-foreground truncate">{{ file.name }}</span>
              </div>
            </CardContent>
          </Card>
          <Badge variant="secondary" class="ml-2">{{ formatFileSize(file.size) }}</Badge>
        </li>
      </ul>
    </div>
  </div>
